...
function jsonRPC2Error(reply, type, requestId, message, code) {
    let errorCode = code;
    switch (type) {
        case "InvalidRequest": {
...
async function default_1(fastify, opts) {
    ...
    async function searchActionByHash(trxHash) {
        try {
    ...
    async function searchDeltasByHash(trxHash) {
        try {
            let _hash = trxHash.toLowerCase();
            if (_hash.startsWith("0x")) {
                _hash = _hash.slice(2);
    ...
    async function reconstructBlockFromReceipts(blockNumber, receipts, full) {
        let actions = [];
        ...
        for (const receiptDoc of receipts) {
            ...
            if (receipt['logsBloom']) {
                bloom.or(new bloom_1.default(Buffer.from(receipt['logsBloom'], "hex")));
    ...
    async function getDeltasByTerm(term, value) {
        const termStruct = {};
    ...
    methods.set('eth_call', async ([txParams]) => {
        if (chainIds.includes(opts.chainId) && chainAddr.includes(txParams.to)) {
            ...
            if (tokens.value.length === 1 && tokens.value[0] === zeros) {
                const balances = await Promise.all(users.value.map((user) => {
    ...
    methods.set('trace_filter', async (params) => {
        ...
        for (const param_obj of params) {
            ...
            let fromBlock = param_obj.fromBlock;
            let toBlock = param_obj.toBlock;
            let after = param_obj.after; //TODO what is this?
            let count = param_obj.count;
    ...
    fastify.post('/', async (request, reply) => {
        ...
        if (methods.has(method)) {
            ...
            try {
                ...
                if (e instanceof TransactionError) {
                    common_functions_1.hLog(`VM execution error, reverted: ${e.errorMessage}`, method, JSON.stringify(params, null, 2));
                    let code = 3;
